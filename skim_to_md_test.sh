...
#%%
#add a summary note for a paper
summary_tags = tags + ",#paper_summary"
files_summary = files 
summary_uid = str(str(timestamp) + "." + "0")
summary_note_id = str(summary_uid + " " + "Summary" + "-" + str(os.path.basename(directory)))
summary_fn = (os.path.join(directory,'%s.md'%(summary_note_id)))
#summary = open(sumary,'w')
summary_result = summary_template.render(note_id=summary_note_id, citekey=citekey,ref=ref,pdf_path = pdf_path,tags=summary_tags,uid=uid)
if any(str("Summary") in file_name for file_name in files): 
        pass
else:
